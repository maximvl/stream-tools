...
<script lang="ts">
  ...
  // NEW: aspect ratio cache (width / height) keyed by image URL
  let imageAspects = $state<Record<string, number>>({});

  const random = (min: number, max: number) => Math.random() * (max - min) + min;

  // NEW: preload every unique image so we know its real dimensions before laying out
  async function preloadImages() {
    const unique = [...new Set(images)];
    await Promise.all(
      unique.map((src) => {
        if (imageAspects[src]) return Promise.resolve();
        return new Promise<void>((resolve) => {
          ...
          img.onerror = () => {
            imageAspects[src] = 1; // fallback square
            resolve();
          };
          img.src = src;
        });
  ...
  function generateTiles() {
    ...
    //   const src = pool[i % pool.length];
      const aspect = imageAspects[src] ?? 1; // default square until loaded

      const polaroid = Math.random() < polaroidChance;
      const tape = !polaroid && Math.random() < tapeChance;

      // NEW: size each tile to the image's real aspect ratio
      let width = tileSize;
      let imgHeight = tileSize / aspect;

      if (polaroid) {
        // polaroid padding: 8px top/right/left, 32px bottom
  ...
  $effect(() => {
    // wait for dimensions before first paint so nothing jumps around
    preloadImages().then(() => generateTiles());

    let ro: ResizeObserver | null = null;
    const handleResize = () => generateTiles();
...
<style>
  ...
  .tile {
    position: absolute;
    overflow: visible; /* CHANGED: was hidden; prevents clipping rotated non-square tiles */
    transition:
    ...
    background: rgba(255, 255, 255, 0.03); /* subtle backing for transparent images */
  }
  ...
  .tile img {
    ...
    width: 100%;
    height: 100%;
    /* CHANGED: was object-fit: cover (cropped).
       The tile is now sized to the image's exact aspect ratio,
       so the whole photo is always visible. */
    object-fit: contain;
